...
def spiral_fractal(x=20, n=15, angle=1, w=10, cut_off=2):
    decrease_rate=0.5
    ...
    for i in range(n):
        ...
        right(angle)
        if i%(n//2+1) == 0:
            spiral_fractal(x=x/1.5, n=int(n//1.5), angle=angle, w=w-decrease_rate, cut_off=cut_off)
            width(w)
            set_color(w)
        elif i == n-1:
            spiral_fractal(x=x/1.5, n=int(n//1.5), angle=angle, w=w-decrease_rate, cut_off=cut_off)
...
try:
    ...
    # spiral_fractal(x=50, angle=10, n=50, cut_off=.1)
    input()
except:
