...
// Funktion, um das Datenarray auf die Auswahl zu begrenzen
...
function updateData() {
    ...
    // Update 1.35 - Bugfix
    if (localStorage.getItem("v1.35") == null) {
        changeList = ["playlast"];
        for (var searchTerm in changeList) {
            for (var key in localStorage) {
                if (key.includes(changeList[searchTerm])) {
                    currentData = localStorage.getItem(key);
                    newData = [];
                    currentArray = currentData.split(listSeperator);
                    for (var entry in currentArray) {
                        prefix = currentArray[entry].split("start.html?=")[0];
                        postfix = currentArray[entry].split("#")[1];
                        encodedData = currentArray[entry].split("start.html?=")[1].split("#")[0];
                        decodedData = atob(encodedData);
                        fixedData = btoa(decodedData.replace("//", "/"));
                        newEntry = prefix + "start.html?=" + fixedData + "#" + postfix;
                        newData.push(newEntry)
                    }
                    newData = newData.join(listSeperator);
                    localStorage.setItem(key, newData);
                }
            }
        }
        localStorage.setItem("v1.35", true);
    }
}
